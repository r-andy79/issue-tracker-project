...
from tickets.models import (
    Ticket, Comment, Vote,
    Payment, BugTicket, FeatureTicket
)


class TicketAdmin(admin.ModelAdmin):
    def save_model(self, request, obj, form, change):
        """Detects status change and sends mail to ticket author"""
        old_obj = Ticket.objects.get(id=obj.id)
        if old_obj.ticket_status != obj.ticket_status:
            try:
                send_mail(
                    f'{obj.title} - change of status',
                    f'Dear {obj.ticket_author.username}, '
                    f'please be advised that the status of your ticket '
                    f'has changed to {obj.get_ticket_status_display()}',
